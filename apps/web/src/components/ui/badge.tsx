...
interface BadgeProps extends HTMLAttributes<HTMLSpanElement> {
  variant?: 'success' | 'warning' | 'error' | 'danger' | 'info' | 'neutral' | 'brand' | 'secondary' | 'default'
}
  ...
  return (
    <span
      className={cn(
        ...
        (variant === 'error' || variant === 'danger') && 'bg-red-500/10 text-red-400 border border-red-500/20',
        variant === 'info' && 'bg-blue-500/10 text-blue-400 border border-blue-500/20',
        (variant === 'neutral' || variant === 'secondary' || variant === 'default') && 'bg-white/5 text-[var(--text-secondary)] border border-white/10',
